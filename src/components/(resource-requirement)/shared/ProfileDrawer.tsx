...
const ProfileDrawer = ({ candidateName, candidateId }: ProfileDrawerProps) => {
  const [data, setData] = useState<ProfileDataType | null>(null);
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);

  if (!candidateId && !candidateName) {
    console.log("coming from the drawer", candidateId, candidateName);
    return null;
  }
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      setData(result.data as ProfileDataType);
    } catch (error) {
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetTrigger asChild>
        <Button
          variant="ghost"
          size={"sm"}
          className={`${candidateId?'underline text-muted-foreground text-xs ':'font-semibold hover:underline'} cursor-pointer h-4 px-0 `}
        >
