...
const matchingOptions = React.useMemo(() => {
  ...
  return filteredOptions.filter((opt) => {
    ...
    const matchesSearch = email.includes(lower) || fullName.includes(lower);
              ...
              {matchingOptions.length > 0 ? (
                <CommandGroup>
                  {matchingOptions.map((opt) => {
                    const selected = value.some((v) => v.email === opt.email);
                    return (
                      <CommandItem
                        key={opt.email}
